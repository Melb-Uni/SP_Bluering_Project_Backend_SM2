...
def get_done_contributor_names(team, jira):
    ...
    for i in range(ceil(total / 100)):  # recurring query if count > 100
        ...
        issues = jira.search_issues(
            'project='+team+' AND Status = DONE', maxResults=100, startAt=start)
...
def auto_get_contributions(request):
    ...
    try:
        ...
        for p in allProjects:
            get_contributions(request, p.name.lower(), jira)
...
def get_ticket_count_from_db(request, team):
    try:
        ...
        result = jira.search_issues(
            'project='+jira_url, start, size, json_result=True)
        total = result['total']
        issues = result['issues']
        for i in range(floor(total / 100)):  # recurring query if count > 100
            start = 0 + (i + 1) * 100
            issues += jira.search_issues('project='+jira_url,
                                         start, size, json_result=True)['issues']
        ...
        if len(issues) != 0:
            ...
            for issue in issues:
                ...
                dt = datetime.datetime.strptime(
                    issue['fields']['created'].split('+')[0], '%Y-%m-%dT%H:%M:%S.%f')
                data['all'].append({'id': issue['id'], 'created': dt.date().strftime(
                    "%Y-%m-%d"), 'status': issue['fields']['status']['name']})
